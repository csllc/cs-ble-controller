...
module.exports = class BleDevice extends EventEmitter {
  ...
  static serviceUuids(name) {
    ...
    return Object.values(periphInfo.services).map(s => s.uuid);
  }
  ...
  _discoverGattServices() {
    // Get GATT services: Iterate through the BLE dongle's list of services and
    ...
    .then((promises) => {
      ...
      promises.forEach((promise) => {
        if (promise.status == 'fulfilled') {
          let characteristic = promise.value;
