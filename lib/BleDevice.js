...
// CS Modbus master
// In a typical application BleController (our parent module) will be the 'device' used
// to create the application's Modbus master.
// We instantiate our own in order to execute commands like keySwitch(), watch(),
// unwatch(), etc.; this one is not meant to be exposed to the world.
...
module.exports = class BleDevice extends EventEmitter {
  ...
  constructor(peripheral, server, options) {
    ...
    // Object containing expected service and characteristic UUIDs, and references to them
    // when they're found in this.inspect()
    this.periphInfo = {};
  ...
  _writeCharacteristic( characteristic, value ) {
    this.emit('writeCharacteristic', characteristic.uuid, value);
  ...
  write(data) {
    ...
    if( txChar ) {
      ...
      while( index < data.length )
      {
        ...
        writes.push( this._writeCharacteristic( txChar, chunk ));
        index += chunkSize;
      }
  ...
  /**
   * Enumerate all GATT services advertised by the BLE peripheral, their characteristics,
   * and the characteristics' descriptors. For services and characteristics listed
   * in the base periphInfo object (assigned to this.periphInfo prior to calling this 
  ...
  _discoverGattServices() {
    ...
    .then((services) => {
      // For each GATT service, get its characteristics
    ...
    .then((characteristics) => {
      // For each GATT characteristic of each GATT service, get its descriptors
      ...
      characteristics.forEach((characteristic) => {
        ...
        promises.push(characteristic.getDescriptors());
      });

      return Promise.all(promises);
    })
    .then((descriptors) => {
      // For each descriptor of each GATT characteristic of each GATT service, save it in
      // the gattInfo object.
      
      descriptors = descriptors.flat();
      ...
      descriptors.forEach((descriptor) => {
        ...
        if (matchingService) {
          ...
          if (matchingChar) {
            if (!matchingChar.desc) {
              matchingChar.desc = [];
            }

            matchingChar.desc.push(descriptor);
          }
        }
      ...
      if ((slot >= 0) && (slot < this._watcherMax)) {
        if (length <= watch.maxLen) {
          ...
          .then(() => {
            ...
            this.subscribeChar(charKey, controllerService, this._watcherCb[slot]);
          })
          .then(() => {
  ...
  superWatch(id, addresses, cb) {
    ...
    if (supported.supported) {
      ...
      .then(() => {
        ...
        this.subscribeChar(charKey, controllerService, this._superWatcherCb);
      })
      .then(() => {
